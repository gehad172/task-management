...
const updateTaskSchema = z.object({
  ...
  }).refine((data) => data.title !== undefined || data.description !== undefined || data.priority !== undefined || data.tags !== undefined || data.assignees !== undefined, {
...
router.post("/", authenticate, validate(createTaskSchema), asyncHandler(async (req, res) => {
  ...
  const taskLean = await Task.findById(taskDoc._id).lean();
  ...
  res.status(201).json({
    task: serializeTask({
      _id: taskLean._id as mongoose.Types.ObjectId,
      title: String(taskLean.title),
      description: taskLean.description ? String(taskLean.description) : undefined,
      status: String(taskLean.status),
      priority: String(taskLean.priority),
      position: taskLean.position,
      commentsCount: taskLean.commentsCount ?? undefined,
      attachmentsCount: taskLean.attachmentsCount ?? undefined,
      dueLabel: taskLean.dueLabel ? String(taskLean.dueLabel) : undefined,
      scheduleLabel: taskLean.scheduleLabel ? String(taskLean.scheduleLabel) : undefined,
      progress: typeof taskLean.progress === "number" ? taskLean.progress : undefined,
      highlighted: Boolean(taskLean.highlighted),
      completed: Boolean(taskLean.completed),
      assigneeAvatar: taskLean.assigneeAvatar ? String(taskLean.assigneeAvatar) : undefined,
      assigneeAvatars: (taskLean.assigneeAvatars as string[] | undefined) ?? undefined,
      assignees: taskLean.assignees as any,
      tags: taskLean.tags as any,
      comments: taskLean.comments as any,
    }),
